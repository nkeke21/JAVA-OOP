...
public class Piece {
	...
	private final static int SENTINEL = 4;
	...
	private void storeInformation(){
		HashMap<Integer, Integer> map = new HashMap<>(); //For each x to save min y for it

		//Go through the body to detect length and height of the figure
		for(int i = 0; i < body.length; i ++){
			...
			//Filling the map which saves min y s for each x
			if(!map.containsKey(tmp.x)) {
				map.put(tmp.x, tmp.y); }
			else if(map.get(tmp.x) > tmp.y){
				map.put(tmp.x, tmp.y);
			}
		}

		//Filling the skirt Array
	...
	public boolean equals(Object obj) {
		...
		return checkForEqual(this.body, other.body);
	}

	private boolean checkForEqual(TPoint[] curr, TPoint[] other) {
		int count = 0;
		for(int i = 0; i < curr.length; i ++){
			for(int j = 0; j < other.length; j ++){
				if(curr[i].equals(other[j])){
					count ++;
					break;
				}
			}
	...
	// es for the standard 7 pieces in the pieces array
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece curr = root;
		while (true){
			Piece nextPi = curr.computeNextRotation();
			curr.next = nextPi;
			if(nextPi.equals(root)) {
				curr.next = root;
				break;
			}
			curr = curr.next;
	...
	public Piece computeNextRotation() {
		TPoint[] SwapBody = new TPoint[body.length];
		int currWidth = height;
		for(int i = 0; i < body.length; i ++){
			SwapBody[i] = new TPoint(currWidth - body[i].y, body[i].x);
		}

		return new Piece(SwapBody); // YOUR CODE HERE
	}
